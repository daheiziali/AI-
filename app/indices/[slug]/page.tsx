...
function HistoryChart() {
  const values = tokenHistory.map((point) => point.value);
  const min = Math.min(...values);
  const max = Math.max(...values);
  const padding = Math.max((max - min) * 0.12, 0.02);
  const chartMin = min - padding;
  const chartMax = max + padding;
  const points = tokenHistory.map((point, index) => ({
    ...point,
    x: (index / (tokenHistory.length - 1)) * 920,
    y: 250 - ((point.value - chartMin) / (chartMax - chartMin)) * 210,
  }));
  const line = points.map((point) => `${point.x},${point.y}`).join(" ");
  const labels = [0, 0.25, 0.5, 0.75, 1].map((ratio) => tokenHistory[Math.round((tokenHistory.length - 1) * ratio)]);
  ...
  return (
    <div className="detail-chart-wrap">
      ...
      <svg viewBox="0 0 920 270" role="img" aria-label="LLM Token支出指数历史走势">
      ...
      <div className="detail-chart-stats"><span>{tokenHistory.length} 个数据点</span><span>历史最低 <b>${min.toFixed(3)}</b></span><span>历史最高 <b>${max.toFixed(3)}</b></span></div>
    </div>
  );
}
    ...
    <div className="detail-page">
      ...
      <main className="detail-content">
        ...
        <section className="detail-history">
          <div className="detail-section-head"><div><p className="eyebrow">PRICE HISTORY</p><h2>历史走势</h2></div><span>{item.hasHistory ? "2025.12.01 至今" : `最新更新 ${item.updatedAt}`}</span></div>
          {item.hasHistory ? <HistoryChart /> : <div className="history-unavailable"><LineChart /><strong>历史序列暂未接入</strong><p>当前展示最新公开报价。未获得对应历史数据前，不生成推测走势。</p></div>}
        </section>
