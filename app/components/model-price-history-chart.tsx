  ...
  return (
    <>
      ...
      <div className="detail-chart-wrap model-price-wrap">
        ...
        {hoveredIndex !== null && selected && <div className={`chart-tooltip detail-tooltip purple ${selected.x > 736 ? "align-right" : ""}`} style={{ left: `${(selected.x / 920) * 100}%`, top: `${(selected.y / 270) * 100}%` }}><span>{selected.date.replaceAll("-", ".")}</span><strong>${selected.value.toFixed(4)}</strong><small>USD / 1M tokens</small></div>}
